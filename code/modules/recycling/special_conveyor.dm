...
/obj/machinery/conveyor/splitter/proc/change_split()
	if(!(directions & 8))
		directions |= 8
...
/obj/machinery/conveyor/splitter/proc/rotate_properly(var/reverse = FALSE)
...
/obj/machinery/conveyor/splitter/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_CROWBAR)
		...
		if(!(machine_stat & BROKEN))
			I.play_tool_sound(src)
			rotate_properly(reverse=TRUE)
			to_chat(user, "<span class='notice'>You \"rotate\" [src] the other way.</span>")
...
/obj/item/stack/conveyor_splitter/afterattack(atom/A, mob/user, proximity)
	...
	if(A == user.loc)
		...
		return
	var/obj/machinery/conveyor/splitter/C = new/obj/machinery/conveyor/splitter(A, cdir, id)
	transfer_fingerprints_to(C)
	use(1)
